...
<div class="max-w-2xl mx-auto rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
    <div class="mb-6">
        <h2 class="text-xl font-semibold text-gray-900">Edit Department Management: {{ $department->name }}</h2>
        <p class="mt-1 text-sm text-gray-500">Update department details and reporting manager.</p>
    </div>
    ...
    <form method="POST" action="{{ route('departments.update', $department) }}" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $department->name) }}" required class="form-control">
        ...
        <div>
            ...
            <select name="manager_id" class="form-control">
                ...
                @foreach($managers as $mgr)
                    <option value="{{ $mgr->id }}" {{ old('manager_id', $department->manager_id) == $mgr->id ? 'selected' : '' }}>{{ $mgr->user->name }} ({{ $mgr->employee_code }})</option>
                @endforeach
            </select>
